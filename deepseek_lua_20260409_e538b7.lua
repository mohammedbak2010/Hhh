-- H2N - ULTIMATE SCRIPT (FULLY FIXED - Float + Auto Track)
...
local function startSpeedBoost()
    ...
    speedConn = RunService.Heartbeat:Connect(function()
        ...
        if moveDir.Magnitude > 0.1 then
            hrp.AssemblyLinearVelocity = Vector3.new(moveDir.X * SpeedSettings.NormalSpeed, hrp.AssemblyLinearVelocity.Y, moveDir.Z * SpeedSettings.NormalSpeed)
        else
...
-- ============================================
-- FLOAT (FIXED - STAYS AT HEIGHT 12)
-- ============================================
...
-- ============================================
-- AUTO TRACK (UPGRADED - FULLY FIXED)
-- ============================================
local TrackSettings = {
...
local function StartTrack()
    ...
    if not hrp then 
        ...
        task.spawn(function()
            local timeout = 0
            while TrackSettings.Enabled and not getHRPTrack() and timeout < 50 do
                task.wait(0.1)
                timeout = timeout + 1
            end
    ...
    TrackSettings.TrackConn = RunService.Heartbeat:Connect(function()
        ...
        local yVel = math.clamp(dY * 10, -30, 30)
    ...
    TrackSettings.AntiWallConn = RunService.Heartbeat:Connect(function()
        ...
        if frontRay and frontRay.Distance < 2 then
            ...
            pcall(function()
                hrpPos.AssemblyLinearVelocity = Vector3.new(lookDir.X * -10, currentVel.Y, lookDir.Z * -10)
            end)
        end
    end)
    
    Notify("TRACK STARTED - Chasing enemies")
end
...
local function StartTrackToggle()
    ...
    -- حفظ حالة الميزات لإعادتها لاحقاً
    ...
    Notify("AUTO TRACK ON - Hunting enemies")
end
...
local function initWPParts()
    createWPPart("L1", L1, WP_BLUE)
    createWPPart("L2", L2, WP_BLUE)
    createWPPart("L_END", L_END, WP_BLUE)
    createWPPart("R1", R1, WP_BLUE)
    createWPPart("R2", R2, WP_BLUE)
    createWPPart("R_END", R_END, WP_BLUE)
end
...
local function Save()
    ...
    local data = {
        ...
        TrackSettings = { Enabled = TrackSettings.Enabled, TrackSpeed = TrackSettings.TrackSpeed, LockSpeed = TrackSettings.LockSpeed, ForwardOffset = TrackSettings.ForwardOffset },
        Keys = {
...
end, function() return TrackSettings.Enabled end, "AutoTrack")
ci = ci + 1
...
MakeToggle(visual, "HIT CIRCLE", vi, function(s)
...
Notify("H2N v8.0 - All Features Working")
print("===============================================================")
print("H2N ULTIMATE v8.0 - FULLY FIXED")
print("- Float: Fixed at height 12 (Stays stable)")
print("- Auto Track: Fully fixed (No more hanging)")
