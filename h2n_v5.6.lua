...
local function startSpeedBoost()
    ...
    speedConn = RunService.Heartbeat:Connect(function()
        ...
        if moveDir.Magnitude > 0.1 then
            ...
            -- إذا الفرق كبير اضبط مباشرة، إذا صغير استخدم smooth
...
-- FLOAT (Silent Duels version - يرتفع لارتفاع محدد وينزل بعد 4 ثواني)
local FloatConn = nil
...
local _dropBlockAutoPlay = false  -- يمنع AutoPlay مؤقتاً بعد DROP
local _wfConns = {}
...
local function executeDrop()
    ...
    task.spawn(function()
        ...
        -- بعد انتهاء الـ drop، انتظر ثانية قبل ما يقدر يشغّل AutoPlay
        task.wait(1.0)
...
local function StartTrack()
    ...
    TrackSettings.TrackConn = RunService.Heartbeat:Connect(function()
        ...
        if flatDist > 1.5 then
            -- تنبؤ بموقع العدو بعد 0.1 ثانية لتقليل الـ lag
        ...
        -- smooth blend لتقليل الـ jitter
...
-- PrimeHub AutoPlay coordinates (updated)
local AP_L1     = Vector3.new(-476,    -7,      93)    -- l1 (updated)
local AP_LEND   = Vector3.new(-484,    -5,      96)    -- l end (updated)
local AP_LFINAL = Vector3.new(-475,    -7,      16)    -- LFIN
local AP_R1     = Vector3.new(-476,    -7,      28)    -- r1 (updated)
local AP_REND   = Vector3.new(-484,    -5,      24)    -- r end (updated)
local AP_RFINAL = Vector3.new(-476,    -7,      105)   -- RFIN
...
WP_META.COLORS = {L1=WP_META.BLUE, L2=WP_META.BLUE, L_END=WP_META.BLUE, R1=WP_META.BLUE, R2=WP_META.BLUE, R_END=WP_META.BLUE}
...
local function StopAutoPlayLeft(byWin)
    ...
    if speedBoostWasOnBeforeAutoPlay and not byWin then
...
local function StopAutoPlayRight(byWin)
...
local function stopFeaturesOnDamage()
    ...
    if DmgState.cooldownTimer then
        pcall(function() task.cancel(DmgState.cooldownTimer) end)
        DmgState.cooldownTimer = nil
    end
    DmgState.cooldownTimer = task.delay(DmgState.COOLDOWN, function()
...
local function stopUnwalk()
    if not UnwalkState.active then return end
    if UnwalkState.animConn then
        UnwalkState.animConn:Disconnect()
        UnwalkState.animConn = nil
    end
...
local function forceRecoverFromRagdoll(hum, root)
    ...
    if ARState.recoveryTimer then
        task.cancel(ARState.recoveryTimer)
        ARState.recoveryTimer = nil
    end
    ...
    ARState.recoveryTimer = task.delay(0.3, function()
...
local function StartAntiRagdoll()
    ...
    if ARState.conn then 
        ARState.conn:Disconnect() 
        ARState.conn = nil
    end
...
-- ============================================
-- HIT CIRCLE (BAT AIMBOT CIRCLE) - PURPLE
-- ============================================
...
UIS.InputBegan:Connect(function(input, gpe)
    ...
    elseif KeyEnabled.Unwalk and k == Keys.Unwalk then
        if UnwalkState.active then stopUnwalk() else startUnwalk() end
...
local SideButtonShape = "square" -- "square" | "circle" | "rect"
...
tabIcons = {Combat="", Protect="", Visual="", Settings=""}
...
ci = ci + 1
MakeToggle(combat, "LOCK", ci, function(s) if s then StartTrackToggle() else StopTrackToggle() end end, function() return TrackSettings.Enabled end, "AutoTrack")
ci = ci + 1
...
MakeToggle(combat, "OPTIMIZER", ci, function(s) if s then startOptimizer() else stopOptimizer() end end, function() return State.Optimizer end, "Optimizer")
ci = ci + 1
...
pi = pi + 1
MakeToggle(protect, "UNWALK (Animations)", pi, function(s) if s then startUnwalk() else stopUnwalk() end end, function() return UnwalkState.active end, "Unwalk")
pi = pi + 1
...
vi = vi + 1
local sideNames = {"AUTO PLAY LEFT", "AUTO PLAY RIGHT", "FLOAT", "SPEED BOOST", "LOCK", "DROP", "AUTO TP DOWN"}
...
MakeNumberBox(visual, "Side Button Size", SideButtonSize, vi, function(val)
    ...
    for _, b in pairs(gui:GetChildren()) do
        if b:IsA("Frame") and b.Name == "SideButton" then
            local btnW = (SideButtonShape == "rect") and math.floor(SideButtonSize * 1.45) or SideButtonSize
...
do -- Button Shape Picker
    ...
    Instance.new("UIStroke", shapeRow).Color = Color3.fromRGB(45,38,80)
    local shapeLbl = Instance.new("TextLabel", shapeRow)
    shapeLbl.Size = UDim2.new(0.4,0,1,0)
    ...
    local shapes = { {id="square", label="■"}, {id="circle", label="●"}, {id="rect", label="▬"} }
    local shapeBtns = {}
    ...
    for i, sh in ipairs(shapes) do
        ...
        sb.Size = UDim2.new(0,36,0,26)
        sb.Position = UDim2.new(1, -10 - (3-i+1)*42, 0.5, -13)
        ...
        Instance.new("UIStroke", sb).Color = Color3.fromRGB(80,60,160)
        table.insert(shapeBtns, { id = sh.id, btn = sb })
        sb.MouseButton1Click:Connect(function()
            ApplyShapeToAllSideButtons(sh.id)
            refreshShapeBtns()
        end)
    end
    refreshShapeBtns()
...
local function GetCornerForShape(shape)
    ...
    elseif shape == "rect" then return UDim.new(0, 6)
    else return UDim.new(0, 14) end -- square (default)
end

local _allSideButtonCorners = {}  -- { corner = UICorner, btn = Frame }
...
local function ApplyShapeToAllSideButtons(shape)
    ...
    for _, data in ipairs(_allSideButtonCorners) do
        ...
        -- مستطيل: عرض أكبر من الارتفاع
        if shape == "rect" then
            data.btn.Size = UDim2.new(0, math.floor(SideButtonSize * 1.45), 0, SideButtonSize)
        else
...
local function CreateSideButton(text, side, index, getState, startFn, stopFn)
    ...
    elseif text == "LOCK" then descText = "Track"
...
CreateSideButton("LOCK", "right", 0, function() return TrackSettings.Enabled end, StartTrackToggle, StopTrackToggle)
...
for _, b in pairs(gui:GetChildren()) do
    if b:IsA("Frame") and b.Name == "SideButton" then
        ...
        -- تطبيق الشكل المحفوظ على الـ UICorner
...
task.spawn(function()
    ...
    for _, b in pairs(gui:GetChildren()) do
        if b:IsA("Frame") and b.Name == "SideButton" then
            ...
            elseif id == "LOCK" then isOn = TrackSettings.Enabled
...
Notify("H2N v5.6 - Shape Picker + Fixes")
print("===============================================================")
print("H2N ULTIMATE v5.6")
print("+ تحديث إحداثيات L1 / LEND / R1 / REND")
print("+ Speed Boost لا يرجع عند الفوز بـ AutoPlay")
print("+ DROP لا يطفي Unwalk")
print("+ AutoPlay محجوب لمدة 1s بعد DROP")
print("+ تم تغيير اسم AUTO TRACK إلى LOCK")
print("+ زر شكل الأزرار: مربع / دائري / مستطيل (Visual tab)")
print("===============================================================")end -- GUI element locals block
