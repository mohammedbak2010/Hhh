...
local function startSpeedBoost()
    ...
    speedConn = RunService.Stepped:Connect(function()
        ...
        if moveDir.Magnitude > 0.1 then
            -- نأخذ الاتجاه الفعلي ونطبق السرعة بشكل مباشر بدون smoothing
...
local function EnhancedGrabLoop(dt)
    ...
    if targetPrompt then
        -- مؤقت دقيق: نجمع dt مباشرة بدون tick()
...
local function executeDrop()
    ...
    -- 35% من القوة الأصلية (300→105, 650→228)
...
local function StartAutoPlayLeft()
    if State.AutoPlayLeft then return end  -- منع التشغيل المزدوج فقط
    if State.AutoPlayRight then StopAutoPlayRight() end
...
local function StartAutoPlayRight()
    if State.AutoPlayRight then return end  -- منع التشغيل المزدوج فقط
    if State.AutoPlayLeft then StopAutoPlayLeft() end
    if TrackSettings.Enabled then StopTrackToggle() end
    if isSpeedBoostEnabled then stopSpeedBoost() end
...
local function CreateSideButton(text, side, index, getState, startFn, stopFn)
    ...
    local function handleRelease(fromSafetyNet)
        if not pressing then return end
        -- نلتقط الحالة قبل reset عشان نقرر مرة وحدة فقط
        local didMove   = hasMoved
        local savedPos  = btn.Position
        local wasOn     = getState()   -- snapshot الحالة الحالية
        resetState()
        if not didMove then
            -- toggle واحد فقط بناءً على snapshot
            if wasOn then stopFn() else startFn() end
            RefreshVisual()
        elseif not fromSafetyNet then
    ...
    -- Safety net: لو الإصبع طار برا الزر (ما ينفذ toggle، بس يحفظ الموضع)
