...
local function executeDrop()
    ...
    task.spawn(function()
        while DropState.active and (tick() - startTime) < DURATION do
...
local function initWPParts()
    createWPPart("L1",L1,WP_META.COLORS.L1)
    createWPPart("L2",L2,WP_META.COLORS.L2)
    createWPPart("L_END",L_END,WP_META.COLORS.L_END)
    createWPPart("R1",R1,WP_META.COLORS.R1)
    createWPPart("R2",R2,WP_META.COLORS.R2)
    createWPPart("R_END",R_END,WP_META.COLORS.R_END)
end
...
MakeToggle(combat, "HIT CIRCLE", ci, function(s) if s then StartHitCircle() else StopHitCircle() end end, function() return HitCircleState.Enabled end)
ci = ci + 1
MakeToggle(combat, "SPAM BAT", ci, function(s) if s then StartSpamBat() else StopSpamBat() end end, function() return SpamBatState.enabled end)
ci = ci + 1

do
    local optRow = Instance.new("Frame", combat)
    optRow.Size = UDim2.new(1,-10,0,40)
    optRow.Position = UDim2.new(0,5,0,ci*44+4)
    optRow.BackgroundColor3 = Color3.fromRGB(18,16,28)
    optRow.BackgroundTransparency = 0
    optRow.ZIndex = 58
    Instance.new("UICorner", optRow).CornerRadius = UDim.new(0,8)
    local optRowStroke = Instance.new("UIStroke", optRow)
    optRowStroke.Color = Color3.fromRGB(45,38,80)
    optRowStroke.Thickness = 1
    local optLbl = Instance.new("TextLabel", optRow)
    optLbl.Size = UDim2.new(0.58,0,1,0)
    optLbl.Position = UDim2.new(0,10,0,0)
    optLbl.BackgroundTransparency = 1
    optLbl.Text = "OPTIMIZER"
    optLbl.TextColor3 = Color3.fromRGB(210,200,240)
    optLbl.Font = Enum.Font.GothamBold
    optLbl.TextSize = 13
    optLbl.TextXAlignment = Enum.TextXAlignment.Left
    local optBtn = Instance.new("TextButton", optRow)
    optBtn.Size = UDim2.new(0,70,0,26)
    optBtn.Position = UDim2.new(1,-78,0.5,-13)
    optBtn.BackgroundColor3 = Color3.fromRGB(70,45,190)
    ...
    local optBtnStk = Instance.new("UIStroke", optBtn)
    optBtnStk.Color = Color3.fromRGB(130,100,255)
    optBtnStk.Thickness = 1
    optBtn.MouseButton1Click:Connect(function()
        optBtn.BackgroundColor3 = Color3.fromRGB(40,160,100)
        optBtn.Text = "DONE"
        RunOptimizer()
        task.wait(1.5)
        optBtn.BackgroundColor3 = Color3.fromRGB(70,45,190)
...
Notify("H2N v5.4 - Fixed Track + Float")
print("===============================================================")
print("H2N ULTIMATE v5.3 - Silent Duels Edition")
print("Added: HitCircle (purple), SpamBat, Optimizer, Silent Duels coords")
