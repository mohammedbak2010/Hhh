-- H2N ULTIMATE - With Auto TP Down
...
-- ========== [ COLORS ] ==========
local Colors = {
    White      = Color3.fromRGB(14, 14, 20),
    LightGray  = Color3.fromRGB(22, 22, 32),
    MediumGray = Color3.fromRGB(90, 60, 220),
    DarkGray   = Color3.fromRGB(70, 45, 190),
    VeryDark   = Color3.fromRGB(8, 8, 14),
    AlmostBlack= Color3.fromRGB(5, 5, 10),
    Border     = Color3.fromRGB(110, 80, 255),
    Text       = Color3.fromRGB(230, 225, 255),
    SubText    = Color3.fromRGB(150, 130, 220),
    Success    = Color3.fromRGB(60, 210, 130),
    Error      = Color3.fromRGB(255, 75, 75),
    DotOff     = Color3.fromRGB(35, 30, 60),
    DotOn      = Color3.fromRGB(130, 100, 255),
    DiscordBlue= Color3.fromRGB(140, 110, 255),
}
...
LP.CharacterAdded:Connect(function(c) task.wait(0.1); Setup(c); floatReadyTick = tick() + 0.5 end)
...
local function pauseSpeed() end
local function resumeSpeed() end
...
local function GetClosestPromptEnhanced()
    ...
    local closestDist = EnhancedGrab.Radius
            ...
            for _, desc in pairs(plot:GetDescendants()) do
                if desc:IsA("ProximityPrompt") and desc.Enabled then
                    ...
                    if not isOwn then
                        ...
                        if parent:IsA("BasePart") then promptPos = parent.Position
                        elseif parent:IsA("Attachment") then promptPos = parent.WorldPosition
                        elseif parent:IsA("Model") then
                            local primary = parent.PrimaryPart or parent:FindFirstChildWhichIsA("BasePart")
                            if primary then promptPos = primary.Position end
                        else
                            local part = parent:FindFirstChildWhichIsA("BasePart")
                            if part then promptPos = part.Position end
                        end
                        if promptPos then
                            local dist = (playerPos - promptPos).Magnitude
...
local function ExecuteStealEnhanced(prompt)
...
local function EnhancedGrabLoop(dt)
    ...
    if targetPrompt then
        ...
        if EnhancedGrab._timer >= EnhancedGrab.Delay then
            ...
            ExecuteStealEnhanced(targetPrompt)
        end
    else
        EnhancedGrab._timer = 0
        EnhancedGrab.CurrentProgress = 0
...
local function StartTrack()
    ...
    TrackSettings.TrackConn = RunService.Heartbeat:Connect(function()
        ...
        if not TrackSettings.AutoBatActive then
            startAutoBatSpam()
        end
...
local function StartTrackToggle()
    ...
    if hrp then 
        ...
        task.spawn(function() 
            while TrackSettings.Enabled and not getHRPTrack() do 
                task.wait(0.1) 
            end 
    ...
    Notify("🔵 AUTO TRACK ON (with Auto Bat)")
end
...
-- ========== [ UNWALK (locks movement + attack animations) ] ==========
local unwalkActive = false
local originalAnimateState = nil
...
local function startUnwalk()
    ...
    if animateScript and animateScript:IsA("LocalScript") then
        originalAnimateState = animateScript.Disabled
...
local function stopUnwalk()
    ...
    if animateScript and animateScript.Parent then
        if originalAnimateState ~= nil then
            animateScript.Disabled = originalAnimateState
        else
    ...
    unwalkActive = false
    animateScript = nil
    Notify("✅ UNWALK OFF (animations restored)")
end
...
local function SaveTPSettings()
    local data = { Enabled = TPSettings.Enabled, TPHeight = TPSettings.TPHeight }
    pcall(function() writefile("H2N_TPDown.json", HttpService:JSONEncode(data)) end)
end

local function LoadTPSettings()
    local ok, raw = pcall(readfile, "H2N_TPDown.json")
    if not ok or not raw then return end
    local ok2, d = pcall(function() return HttpService:JSONDecode(raw) end)
    if ok2 then
        if d.Enabled ~= nil then TPSettings.Enabled = d.Enabled end
        if d.TPHeight then TPSettings.TPHeight = d.TPHeight end
    end
end

LoadTPSettings()
if TPSettings.Enabled then StartTPMonitoring() end
...
local KeyEnabled = {
    ...
    AntiRagdoll = true, Float = true, SpeedBoost = true, Unwalk = true, AutoTPDown = true,
}
...
local function Save()
    ...
    local data = {
        ...
        KeyEnabled = {
            ...
            Unwalk = KeyEnabled.Unwalk,
            AutoTPDown = KeyEnabled.AutoTPDown,
        },
        ...
        TPSettings = {
            Enabled = TPSettings.Enabled,
            TPHeight = TPSettings.TPHeight,
        },
    }
    ...
    task.defer(function()
        for id, boxRef in pairs(numberBoxReferences) do
            if boxRef and boxRef.TextBox then
                ...
                elseif id == "TPHeight" then
                    boxRef.TextBox.Text = tostring(TPSettings.TPHeight)
                end
            end
        end
...
-- Version label on right of header
...
-- Tab icons map
local tabIcons = {Combat="⚔", Protect="🛡", Visual="👁", Settings="⚙"}
...
for i, name in ipairs(tabNames) do
    ...
    tb.MouseButton1Click:Connect(function()
        ...
        for _, b in pairs(tabBtns) do
            ...
            Instance.new("UIStroke", b).Color = Color3.fromRGB(50, 40, 90)
        end
        sf.Visible = true
...
MakeKeybind(sTab, "Unwalk Key (U)", "Unwalk", si); si = si + 1
MakeKeybind(sTab, "Auto TP Down Key (T)", "AutoTPDown", si); si = si + 1

-- ====== SIDE BUTTONS (7) - AUTO TRACK PRIORITY SYSTEM ======

-- ========== [ PRIORITY SYSTEM VARIABLES ] ==========
local speedBoostWasOnBeforeTrack = false
local tpDownWasOnBeforeTrack = false

-- ========== [ AUTO TRACK PRIORITY FUNCTIONS ] ==========
local _OriginalStartTrackToggle = StartTrackToggle
local _OriginalStopTrackToggle  = StopTrackToggle

StartTrackToggle = function()
    if TrackSettings.Enabled then return end
    speedBoostWasOnBeforeTrack = isSpeedBoostEnabled
    tpDownWasOnBeforeTrack     = TPSettings.Enabled
    if isSpeedBoostEnabled then
        _OriginalStopSpeedBoost()
        Notify("⚫ SPEED BOOST paused (AUTO TRACK priority)")
    end
    if TPSettings.Enabled then
        TPSettings.Enabled = false
        StopTPMonitoring()
        Notify("⚫ AUTO TP DOWN paused (AUTO TRACK priority)")
    end
    if State.FloatEnabled then stopFloat() end
    _OriginalStartTrackToggle()
end

StopTrackToggle = function()
    if not TrackSettings.Enabled then return end
    _OriginalStopTrackToggle()
    if speedBoostWasOnBeforeTrack then
        speedBoostWasOnBeforeTrack = false
        _allowSpeedBoostRestore = true
        startSpeedBoost()
...
-- ========== [ SPEED BOOST priority wrapper - blocks during TRACK ] ==========
local _OriginalStartSpeedBoost = startSpeedBoost
local _OriginalStopSpeedBoost  = stopSpeedBoost
local _allowSpeedBoostRestore = false

startSpeedBoost = function()
    if TrackSettings.Enabled and not _allowSpeedBoostRestore then
        Notify("⚠️ Cannot start SPEED BOOST while AUTO TRACK is active")
        return
    end
    _allowSpeedBoostRestore = false
    _OriginalStartSpeedBoost()
end

stopSpeedBoost = function()
    _OriginalStopSpeedBoost()
end

-- ========== [ TP DOWN - blocked during TRACK ] ==========
local function StartAutoTPDown_Priority()
    if TrackSettings.Enabled then
        Notify("⚠️ Cannot start AUTO TP DOWN while AUTO TRACK is active")
        return
    end
    TPSettings.Enabled = true
    StartTPMonitoring()
...
local function StopAutoTPDown_Priority()
    TPSettings.Enabled = false
    StopTPMonitoring()
    SaveTPSettings()
...
-- ========== [ DROP with full priority system ] ==========
local _OriginalExecuteDrop = executeDrop

local function HandleDropPress_Priority()
    if TrackSettings.Enabled then
        StopTrackToggle()
    end
    if State.AutoPlayLeft  then StopAutoPlayLeft()  end
...
local function CreateSideButton(text, side, index, getState, startFn, stopFn)
    ...
    elseif text == "AUTO TRACK"      then descText = "Track (Priority)"
    ...
    local function handleRelease()
        ...
        resetState()
        if not didMove then
            if wasOn then stopFn() else startFn() end
            RefreshVisual()
        else
...
-- ========== [ CREATE SIDE BUTTONS (7) with priority ] ==========
...
CreateSideButton("AUTO TP DOWN", "right", 2, function() return TPSettings.Enabled    end, StartAutoTPDown_Priority, StopAutoTPDown_Priority)
CreateSideButton("FLOAT",        "right", 3, function() return State.FloatEnabled    end, startFloat,       stopFloat)
CreateSideButton("DROP",         "right", 4, function() return false                 end, HandleDropPress_Priority, function() end)
...
if EnhancedGrab.Enabled then
    task.spawn(function()
        ...
        EnhancedGrab.Enabled = false
        StartEnhancedGrab()
        print("✅ Auto Grab restored from save")
    end)
end
...
if TrackSettings.Enabled then
    task.spawn(function()
        task.wait(2.5)
        StartTrackToggle()
        print("✅ Auto Track restored from save")
    end)
end
...
Notify("H2N Ultimate | ✅ Auto Track (Priority) | ✅ Unwalk | ✅ Auto TP Down | ✅ 7 Side Buttons")
print("=" .. string.rep("=", 65))
print("✅ H2N ULTIMATE - VERSION FINAL")
print("1. ✅ New Coordinates")
print("2. ✅ Auto Track (ExtraForward=0.45 + Auto Bat)")
print("3. ✅ Unwalk (locks movement + attack animations)")
print("4. ✅ Auto TP Down (lands at height 12.5 - key T)")
print("5. ✅ 7 Side Buttons (with TP Down)")
print("=" .. string.rep("=", 65))
print("📌 KEYBINDS:")
print("✓ G = Auto Play Left | H = Auto Play Right | F = Float")
print("✓ B = Speed Boost | J = Infinite Jump | K = Anti Ragdoll")
print("✓ U = Unwalk | T = Auto TP Down")
print("=" .. string.rep("=", 65))