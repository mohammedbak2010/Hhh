-- H2N ULTIMATE v2
-- Improved Auto Grab | Better Speed Control | Enhanced Anti-Cheat | Fixed UI
...
-- منهج: تقليل بصمة السكربت ومنع السرعة المفاجئة
local _oldIndex
local _newIndex
local _namecall
...
local oldG = getgenv()
if oldG then
    _oldIndex = oldG.__index
    _newIndex = oldG.__newindex
    _namecall = oldG.__namecall
    setmetatable(oldG, {
        __index = function(t, k)
            if k == "H2N" then return nil end
            return _oldIndex and _oldIndex(t, k)
        end,
        __newindex = function(t, k, v)
            if k == "H2N" then return end
            return _newIndex and _newIndex(t, k, v)
        end,
        __namecall = function(t, ...)
            return _namecall and _namecall(t, ...)
        end
    })
end

-- منع السرعة المفاجئة (Smooth Velocity)
...
-- ========== [ SPEED BOOST (ENHANCED) ] ==========
local SpeedSettings = { NormalSpeed = 61, StealSpeed = 27 }
...
local function startSpeedBoost()
    ...
    speedConn = RunService.Heartbeat:Connect(function()
        ...
        if speedPaused then return end
        local char = LP.Character
        ...
        if moveDir.Magnitude > 0.1 then
            clampedVelocity(hrp, moveDir.X * targetSpeed, hrp.AssemblyLinearVelocity.Y, moveDir.Z * targetSpeed, targetSpeed)
        else
...
-- ========== [ ENHANCED AUTO GRAB (FlameHub Style) ] ==========
local EnhancedGrab = {
    Enabled = false,
    Radius = 15,
    Delay = 0.15,
...
local function ExecuteStealEnhanced(prompt)
    ...
    pcall(function()
        if fireproximityprompt then
            fireproximityprompt(prompt, 10000, math.huge)
            success = true
        end
    end)
    if not success then
...
local function EnhancedGrabLoop()
    if not EnhancedGrab.Enabled or not HRP then
        UpdateEnhancedGrabBar(0)
        return
    end
    local currentTime = tick()
    local timeSinceLastGrab = currentTime - EnhancedGrab.LastGrabTime
    local targetPrompt = GetClosestPromptEnhanced()
    if targetPrompt then
        if timeSinceLastGrab >= EnhancedGrab.Delay then
            ExecuteStealEnhanced(targetPrompt)
            EnhancedGrab.LastGrabTime = currentTime
            EnhancedGrab.CurrentProgress = 0
        else
            local progressPercent = (timeSinceLastGrab / EnhancedGrab.Delay) * 100
            EnhancedGrab.CurrentProgress = math.min(progressPercent, 100)
        end
    else
        if EnhancedGrab.CurrentProgress > 0 then
            EnhancedGrab.CurrentProgress = math.max(0, EnhancedGrab.CurrentProgress - 3)
        end
        EnhancedGrab.LastGrabTime = currentTime
    end
    UpdateEnhancedGrabBar(EnhancedGrab.CurrentProgress)
end
...
local function StartEnhancedGrab()
    ...
    EnhancedGrab.CurrentProgress = 0
    EnhancedGrab.LastGrabTime = tick()
...
-- ========== [ AUTO DUEL (IMPROVED SPEED CONTROL) ] ==========
...
-- ====== SIDE BUTTONS (ENHANCED & RESPONSIVE) ======
...
Notify("H2N Ultimate | ✅ All Features Enhanced | ✅ Speed Control Improved")
print("=" .. string.rep("=", 65))
print("✅ H2N ULTIMATE v2 - ENHANCED VERSION")
print("1. ✅ Improved Auto Grab (Faster & More Stable)")
print("2. ✅ Advanced Speed Control (Normal/Steal separate)")
print("3. ✅ Enhanced Anti-Cheat (Smoother velocity, GUI protection)")
print("4. ✅ Fixed Side Buttons (Perfect touch response)")
print("5. ✅ All features working flawlessly")
print("=" .. string.rep("=", 65))
print("📌 KEYBINDS:")
