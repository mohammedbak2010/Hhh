...
local function startFloat()
    ...
    -- AutoTrack يطفي عند تشغيل Float (يستخدم pcall لأن StopTrackToggle معرّفة لاحقاً)
    pcall(function()
...
-- ===== V3 Grab: كاش الكولباكات لكل برومبت =====
...
local function executeDrop()
    ...
    task.spawn(function()
        ...
        -- انتظر شوي بعد انتهاء الـ drop عشان الـ velocity يشتغل بسلاسة قبل ما يرجع speed boost
...
local _switchingModes = false -- منع الـ recursive loop عند التبديل بين AutoPlay وAutoTrack
...
local function StartAutoPlayLeft()
    ...
    -- لو AutoTrack شغال، طفّه وشغّل AutoPlay
...
ci = ci + 1
MakeNumberBox(combat, "Grab Speed", EnhancedGrab.Delay, ci, function(v) EnhancedGrab.Delay = math.clamp(v,0.05,2.0); if grabBarRef.rateLbl then grabBarRef.rateLbl.Text = string.format("%.2f", EnhancedGrab.Delay).."s" end; Notify("Grab Speed = "..string.format("%.2f", EnhancedGrab.Delay).."s") end, 0.05, 2.0, "GrabDelay")
ci = ci + 1
...
print("===============================================================")
print("H2N")
print("AUTO PLAY <-> AUTO TRACK تبديل تلقائي في الاتجاهين")
print("===============================================================")