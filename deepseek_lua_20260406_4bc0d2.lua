...
local TrackSettings = {
    ...
    LockSpeed = 80,
    TrackSpeed = 58,
    ForwardOffset = 0.8,        -- ✅ تم التعديل: 0.8 بدلاً من 2.3 (يقف قدام الخصم بشوي)
    VerticalAim = true,          -- ✅ تفعيل التوجيه العمودي (Y)
    TrackConn = nil,
    AntiWallConn = nil,
    AlignOri = nil,
    Attachment = nil,
    AutoBatActive = false,       -- ✅ تفعيل Auto Bat تلقائياً مع Auto Track
    AutoBatLoop = nil,
}
...
local function StartTrack()
    ...
    TrackSettings.TrackConn = RunService.Heartbeat:Connect(function()
        ...
        -- ✅ الهدف: أمام الخصم مباشرة مع تعديل بسيط لمسافة الوقوف
        local aimX = target.Position.X + offsetX
        local aimZ = target.Position.Z + offsetZ
        
        -- ✅ دعم المحور Y (عمودي) إذا كان الخصم أعلى أو أسفل
        local aimY = target.Position.Y
        if TrackSettings.VerticalAim then
            aimY = target.Position.Y + 1.5 -- نهدف نحو منتصف جسم الخصم
        end
        ...
        -- ✅ توجيه الجسم نحو الخصم مع مراعاة Y
        if TrackSettings.AlignOri then
            local dirX = target.Position.X - h.Position.X
            local dirY = (target.Position.Y + 1.2) - h.Position.Y
            local dirZ = target.Position.Z - h.Position.Z
            local dirMag = math.sqrt(dirX * dirX + dirY * dirY + dirZ * dirZ)
            if dirMag > 0.1 then
                TrackSettings.AlignOri.Enabled = true
                TrackSettings.AlignOri.CFrame = CFrame.lookAt(h.Position, h.Position + Vector3.new(dirX / dirMag, dirY / dirMag, dirZ / dirMag))
            end
        end
    end)
    
    -- ✅ منع الاصطدام بالجدران
...
local function StopTrack()
    ...
    -- ✅ إيقاف Auto Bat عند إيقاف Auto Track
    stopAutoBatSpam()
...
local function StartTrackToggle()
    ...
    Notify("🔵 AUTO TRACK ON (مع Auto Bat و Aim Y)")
end
...
local function startUnwalk()
    ...
    -- ✅ منع أي قوة خارجية من تحريك اللاعب
    if unwalkConnection then unwalkConnection:Disconnect() end
...
local function stopUnwalk()
    ...
    Notify("✅ UNWALK OFF (حركة مفعلة)")
end

-- ========== [ WAYPOINTS - الإحداثيات الجديدة ] ==========
...
local Keys = {
    ...
    Unwalk = Enum.KeyCode.U,  -- ✅ إضافة مفتاح Unwalk
}
local KeyEnabled = {
    InfJump = true, AutoPlayLeft = true, AutoPlayRight = true,
    AntiRagdoll = true, Float = true, SpeedBoost = true, Unwalk = true,
}
...
local sideNames = {"AUTO PLAY LEFT", "AUTO PLAY RIGHT", "FLOAT", "SPEED BOOST", "AUTO TRACK", "DROP", "UNWALK"}
...
local function CreateSideButton(text, side, index, getState, startFn, stopFn)
    ...
    elseif text == "UNWALK" then descText = "تجميد"
    end
...
-- إنشاء الأزرار الجانبية (7 أزرار)
...
CreateSideButton("UNWALK", "right", 4, function() return unwalkActive end, startUnwalk, stopUnwalk)
...
task.spawn(function()
    ...
    for _, b in pairs(gui:GetChildren()) do
        if b:IsA("Frame") and b.Name == "SideButton" then
            ...
            elseif id == "UNWALK" then
                b.BackgroundColor3 = unwalkActive and Colors.DarkGray or Colors.White
            end
        end
    end
...
Notify("H2N Ultimate | ✅ Menu Fixed | ✅ New Coordinates | ✅ Fast Response | ✅ Unwalk | ✅ Auto Bat inside Auto Track")
print("=" .. string.rep("=", 65))
print("✅ H2N ULTIMATE - UPDATED")
print("1. ✅ New Coordinates:")
print("   L2 = (-485, -5, 95)")
print("   L_END = (-475, -7, 18)")
print("   R2 = (-485, -5, 26)")
print("   R_END = (-476, -7, 101)")
print("2. ✅ Fast button response (no conflict with Speed Boost)")
print("3. ✅ Auto Track محسن: يقف أمام الخصم (ForwardOffset=0.8) + دعم Y")
print("4. ✅ Auto Bat مدمج داخل Auto Track (ضرب تلقائي)")
print("5. ✅ ميزة UNWALK جديدة (زر U أو الزر الجانبي)")
print("=" .. string.rep("=", 65))
print("📌 KEYBINDS:")
print("✓ G = Auto Play Left | H = Auto Play Right | F = Float")
print("✓ B = Speed Boost | J = Infinite Jump | K = Anti Ragdoll | U = Unwalk")
print("=" .. string.rep("=", 65))