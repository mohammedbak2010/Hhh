...
local EnhancedGrab = { Enabled = false, Radius = 10, Delay = 0.20, CurrentProgress = 0, _timer = 0, LoopConnection = nil }
local grabBarRef = {}
local sbFill = nil
...
local function ExecuteStealEnhanced(prompt)
    ...
    local success = false
    pcall(function() fireproximityprompt(prompt); success = true end)
    if not success then
        pcall(function()
...
local function StopTrackToggle()
    ...
    -- بعد إيقاف AutoTrack، شغّل AutoPlay تلقائياً (إلا لو كان التبديل داخلي)
    if not _switchingModes and not State.AutoPlayLeft and not State.AutoPlayRight then
        _switchingModes = true
        task.spawn(function()
            task.wait(0.05)
            -- شغّل AutoPlay Left بشكل افتراضي
...
local function StopAutoPlayLeft()
    ...
    -- بعد إيقاف AutoPlay، شغّل AutoTrack تلقائياً (إلا لو كان التبديل داخلي)
    if not _switchingModes and not State.AutoPlayRight and not TrackSettings.Enabled then
        _switchingModes = true
        task.spawn(function()
            task.wait(0.05)
            StartTrackToggle()
...
local function StopAutoPlayRight()
    ...
    -- بعد إيقاف AutoPlay، شغّل AutoTrack تلقائياً (إلا لو كان التبديل داخلي)
    if not _switchingModes and not State.AutoPlayLeft and not TrackSettings.Enabled then
        _switchingModes = true
        task.spawn(function()
            task.wait(0.05)
            StartTrackToggle()
...
ci = ci + 1
MakeNumberBox(combat, "Grab Speed", EnhancedGrab.Delay, ci, function(v) EnhancedGrab.Delay = math.clamp(v,0.05,1.0); if grabBarRef.rateLbl then grabBarRef.rateLbl.Text = string.format("%.2f", EnhancedGrab.Delay).."s" end; Notify("Grab Speed = "..string.format("%.2f", EnhancedGrab.Delay).."s") end, 0.05, 1.0, "GrabDelay")
ci = ci + 1
