...
-- ============================================
-- FLOAT (FIXED - Stops exactly at height 12)
-- ============================================
...
local function startFloat()
    ...
    FloatConn = RunService.Heartbeat:Connect(function()
        ...
        if currentY < FloatHeight - 0.3 then
            local upSpeed = math.min((FloatHeight - currentY) * 15, FloatUpSpeed)
            hrp.AssemblyLinearVelocity = Vector3.new(hrp.AssemblyLinearVelocity.X, upSpeed, hrp.AssemblyLinearVelocity.Z)
        elseif currentY >= FloatHeight - 0.3 then
            hrp.CFrame = CFrame.new(hrp.Position.X, FloatHeight, hrp.Position.Z)
...
local function EnhancedGrabLoop(dt)
    ...
    if targetPrompt then
        ...
        if EnhancedGrab._timer >= EnhancedGrab.Delay then
            ...
            pcall(function()
                fireproximityprompt(targetPrompt, 0, 0)
            end)
        end
    else
        EnhancedGrab._timer = 0
...
local function StartTrack()
    ...
    TrackSettings.TrackConn = RunService.Heartbeat:Connect(function()
        ...
        if moveMag > 1 then
            ...
            offsetX = normX * (TrackSettings.ForwardOffset + TrackSettings.ExtraForward)
            offsetZ = normZ * (TrackSettings.ForwardOffset + TrackSettings.ExtraForward)
        end
    ...
    TrackSettings.AntiWallConn = RunService.Heartbeat:Connect(function()
        ...
        if frontRay and frontRay.Distance < 2 then
            ...
            hrpPos.AssemblyLinearVelocity = Vector3.new(lookDir.X * -12, currentVel.Y, lookDir.Z * -12)
        end
    end)
end
...
-- ============================================
-- AUTO TRACK TOGGLE (with TP Down and Hit Circle management)
-- ============================================
local hitCircleWasOnBeforeTrack = false
...
local function StartTrackToggle()
    ...
    hitCircleWasOnBeforeTrack = State.HitCircle
    ...
    if State.HitCircle then
        StopHitCircle()
        Notify("HIT CIRCLE paused for AUTO TRACK")
    end
...
local function StopTrackToggle()
    ...
    if hitCircleWasOnBeforeTrack then
        hitCircleWasOnBeforeTrack = false
        StartHitCircle()
        Notify("HIT CIRCLE restored")
    end
...
ci = ci + 1
MakeToggle(combat, "SPAM BAT", ci, function(s) if s then startSpamBat() else stopSpamBat() end; State.SpamBat = s end, function() return State.SpamBat end, "SpamBat")
ci = ci + 1
...
vi = vi + 1
MakeToggle(visual, "HIT CIRCLE", vi, function(s) if s then StartHitCircle() else StopHitCircle() end end, function() return State.HitCircle end, "HitCircle")
vi = vi + 1
MakeToggle(visual, "OPTIMIZER", vi, function(s) if s then startOptimizer() else stopOptimizer() end end, function() return State.Optimizer end, "Optimizer")
vi = vi + 1
...
Notify("H2N v6.0 - All Features Working")
print("===============================================================")
print("H2N ULTIMATE v6.0")
print("- Auto Track: Upgraded with Auto Bat")
print("- Auto Play Left/Right: Waypoints (Working)")
print("- Float: Fixed at height 12 (Stops exactly)")
print("- Auto TP Down: Auto disabled when Track is ON, restored when OFF")
print("- Hit Circle: Auto disabled when Track is ON, restored when OFF")
print("===============================================================")