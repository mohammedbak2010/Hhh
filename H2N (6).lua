...
local DROP_COOLDOWN = 8  -- كولداون أطول عشان ما يُكشف
...
local function executeDrop()
    ...
    -- velocity معقول ومو مفاجئ — بدون BodyVelocity اللي يلفت الانتباه
    local VELOCITY_X = 55   -- كان 300، مخفض بشكل كبير
    local VELOCITY_Y = 90   -- كان 650، مخفض بشكل كبير
    local STEPS = 6         -- تدريجي على عدة frames
    task.spawn(function()
        for i = 1, STEPS do
            if not dropIsActive then break end
            local c = LP.Character
            if not c then break end
            local r = c:FindFirstChild("HumanoidRootPart")
            if not r then break end
            -- تصاعدي: نبدأ بنص السرعة ونوصل للكامل
...
-- يحرك نحو النقطة كل Heartbeat — يرجع true لو وصل
...
local function updateAutoPlayLeft()
    ...
    if reached then
        aplPhase = aplPhase + 1
        if aplPhase > #LEFT_ROUTE then aplPhase = 1 end  -- يلف تلقائي
    end
end
...
local function updateAutoPlayRight()
    ...
    if reached then
        aprPhase = aprPhase + 1
        if aprPhase > #RIGHT_ROUTE then aprPhase = 1 end  -- يلف تلقائي
    end
end
...
local function StartAutoPlayLeft()
    ...
    -- Speed Boost يتعارك مع Auto Play — أوقفه تلقائياً
...
local function CreateSideButton(text, side, index, getState, startFn, stopFn)
    ...
    -- Safety net: لو الإصبع طار برا الزر
