...
-- ========== [ AUTO TRACK (النظام القديم + Auto Bat + تقدم بسيط) ] ==========
local TrackSettings = {
    Enabled = false,
    LockSpeed = 80,
    TrackSpeed = 58,
...
local function StartTrackToggle()
    ...
    Notify("🔵 AUTO TRACK ON (مع Auto Bat)")
end
...
-- ========== [ UNWALK (يزيل أنيميشن الحركة والضرب معاً) ] ==========
...
local function startUnwalk()
    ...
    unwalkActive = true
    Notify("🚫 UNWALK ON (الأنيميشن مقفل - حركة وضرب)")
end
...
local function stopUnwalk()
    ...
    unwalkActive = false
    animateScript = nil
    Notify("✅ UNWALK OFF (الأنيميشن رجع)")
end
...
MakeToggle(visual, "Auto TP Down", vi, function(s) 
...
MakeNumberBox(visual, "TP Height", TPSettings.TPHeight, vi, function(v)
    TPSettings.TPHeight = math.clamp(v, 5, 50)
    SaveTPSettings()
    Notify("TP Height = " .. TPSettings.TPHeight)
end, 5, 50, "TPHeight")
vi = vi + 1
...
-- ========== [ TP DOWN مع منع التشغيل أثناء TRACK ] ==========
local function StartAutoTPDown_Priority()
    if TrackSettings.Enabled then
        Notify("⚠️ لا يمكن تشغيل AUTO TP DOWN أثناء AUTO TRACK (أولوية قصوى)")
        return
    end
    TPSettings.Enabled = true
    StartTPMonitoring()
...
-- ========== [ DROP مع نظام الأولوية الكامل ] ==========
local _OriginalExecuteDrop = executeDrop

local function HandleDropPress_Priority()
    -- DROP يوقف AUTO TRACK ويستعيد SPEED BOOST و TP DOWN
    if TrackSettings.Enabled then
        StopTrackToggle()
    end
    -- إيقاف AUTO PLAY و FLOAT
    if State.AutoPlayLeft  then StopAutoPlayLeft()  end
    if State.AutoPlayRight then StopAutoPlayRight() end
    if State.FloatEnabled  then stopFloat()         end
    -- تنفيذ DROP
...
local function CreateSideButton(text, side, index, getState, startFn, stopFn)
    ...
    elseif text == "AUTO TRACK"      then descText = "تتبع (أولوية)"
    elseif text == "DROP"            then descText = "رمي"
    elseif text == "AUTO TP DOWN"    then descText = "نزول تلقائي"
    end
    ...
    local function RefreshVisual()
        ...
        stroke.Color = getState() and Colors.DotOn or Colors.Border
    end
...
-- ========== [ إنشاء الأزرار الجانبية (7 أزرار) مع نظام الأولوية ] ==========
...
task.spawn(function()
    ...
    for _, b in pairs(gui:GetChildren()) do
        if b:IsA("Frame") and b.Name == "SideButton" then
            ...
            elseif id == "AUTO TP DOWN" then
                b.BackgroundColor3 = TPSettings.Enabled and Colors.DarkGray or Colors.White
            end
        end
    end
...
Notify("H2N Ultimate | ✅ Auto Track (أولوية قصوى) | ✅ Unwalk | ✅ Auto TP Down | ✅ 7 أزرار جانبية")
print("=" .. string.rep("=", 65))
print("✅ H2N ULTIMATE - VERSION FINAL")
print("1. ✅ New Coordinates")
print("2. ✅ Auto Track (النظام القديم + ExtraForward=0.45 + Auto Bat)")
print("3. ✅ Unwalk (يعطل أنيميشن الحركة والضرب معاً)")
print("4. ✅ Auto TP Down (ينزلك عند ارتفاع 12.5 - زر T)")
print("5. ✅ 7 أزرار جانبية (مع TP Down)")
print("=" .. string.rep("=", 65))
print("📌 KEYBINDS:")
