...
local function executeDrop()
    ...
    task.spawn(function()
        -- حماية مؤقتة: نمنع الموت خلال الـ drop
        local char = LP.Character
        local hum = char and char:FindFirstChildOfClass("Humanoid")
        local savedHealth, savedMaxHealth
        if hum then
...
local AP_LEND   = Vector3.new(-483,    -5,      96)    -- l end (updated)
local AP_LFINAL = Vector3.new(-475,    -7,      16)    -- LFIN (updated)
local AP_R1     = Vector3.new(-476,    -6,      28)
local AP_REND   = Vector3.new(-483,    -5,      24)    -- r end (updated)
local AP_RFINAL = Vector3.new(-476,    -7,      105)   -- RFIN (updated)
...
local function Save()
    ...
    local data = {
        SideButtonSize = SideButtonSize, menuW = menuW, menuH = menuH,
...
Notify("H2N v5.5 - Save Fix + Grab Radius + Drop Fix")
print("===============================================================")
print("H2N ULTIMATE v5.5")
print("+ Unwalk / HitCircle / SpamBat states now save correctly")
print("+ Grab Radius number box added below AUTO GRAB toggle")
print("+ DROP no longer kills the player (health protected during fling)")
print("+ Updated waypoint coordinates: l end, LFIN, r end, RFIN")
print("+ WP marker parts show in map for all waypoints")
