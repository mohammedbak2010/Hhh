-- H2N - FULL SCRIPT WITH UNWALK (Menu Only, No Side Button)
...
local State = {
    ...
    SpeedBoostEnabled = false, AutoGrab = false,
}
...
local function startFloat()
    ...
    pcall(function()
        if TrackSettings and TrackSettings.Enabled then
            ...
            if StopTrackToggle then StopTrackToggle() end
...
local function executeDrop()
    ...
    speedBoostWasOnBeforeDrop = isSpeedBoostEnabled
    ...
    local char = LP.Character
    if not char then
        if speedBoostWasOnBeforeDrop then startSpeedBoost() end
        dropIsActive = false
        return
    end
    local hrp = char:FindFirstChild("HumanoidRootPart")
    ...
    task.spawn(function()
        ...
        if speedBoostWasOnBeforeDrop then
            task.wait(POST_DROP_DELAY)
            speedBoostWasOnBeforeDrop = false
            startSpeedBoost()
...
local TPSettings = {
    Enabled = false, TPHeight = 12.5, LastTPTime = 0, TP_COOLDOWN = 0.15,
...
-- ============================================
-- IMPROVED UNWALK (Animations Only - No physics)
-- ============================================
...
local function startUnwalk()
    ...
    unwalkActive = true
    Notify("UNWALK ON - Animations frozen (movement works)")
end
    ...
    if char then
        for _, obj in ipairs(char:GetDescendants()) do
            if obj:IsA("Motor6D") then
                ...
                pcall(function()
                    if obj.C0 then obj.C0 = obj.C0 end
                    if obj.C1 then obj.C1 = obj.C1 end
                end)
            end
        end
    end
...
local function StartAntiRagdoll()
    ...
    antiRagdollConn = RunService.Heartbeat:Connect(function()
        ...
        if isRagdolled or hasBrokenJoints or isSpinning then
            forceRecoverFromRagdoll(hum, root)
        end
    end)
    
    Notify("ANTI RAGDOLL V2 ON")
end
...
-- ============================================
-- HIT CIRCLE (AIM CIRCLE) - بنفسجي
-- ============================================
local HitCircle = { Enabled = false, Conn = nil, Circle = nil, Align = nil, Attach = nil }
local HIT_CIRCLE_COLOR = Color3.fromRGB(160, 80, 255) -- بنفسجي

local function startHitCircle()
    if HitCircle.Enabled then return end
    local char = LP.Character or LP.CharacterAdded:Wait()
    local hrp = char:WaitForChild("HumanoidRootPart")
    local hum = char:FindFirstChildOfClass("Humanoid")

    if HitCircle.Attach then pcall(function() HitCircle.Attach:Destroy() end) end
    if HitCircle.Align  then pcall(function() HitCircle.Align:Destroy()  end) end
    if HitCircle.Circle then pcall(function() HitCircle.Circle:Destroy() end) end

    HitCircle.Attach = Instance.new("Attachment", hrp)
    HitCircle.Align = Instance.new("AlignOrientation", hrp)
    HitCircle.Align.Attachment0 = HitCircle.Attach
    HitCircle.Align.Mode = Enum.OrientationAlignmentMode.OneAttachment
    HitCircle.Align.RigidityEnabled = true

    HitCircle.Circle = Instance.new("Part")
    HitCircle.Circle.Shape = Enum.PartType.Cylinder
    HitCircle.Circle.Material = Enum.Material.Neon
    HitCircle.Circle.Size = Vector3.new(0.05, 14.5, 14.5)
    HitCircle.Circle.Color = HIT_CIRCLE_COLOR
    HitCircle.Circle.CanCollide = false
    HitCircle.Circle.Massless = true
    HitCircle.Circle.Parent = workspace
    local weld = Instance.new("Weld")
    weld.Part0 = hrp
    weld.Part1 = HitCircle.Circle
    weld.C0 = CFrame.new(0, -1, 0) * CFrame.Angles(0, 0, math.rad(90))
    weld.Parent = HitCircle.Circle

    HitCircle.Enabled = true

    if HitCircle.Conn then HitCircle.Conn:Disconnect() end
    HitCircle.Conn = RunService.RenderStepped:Connect(function()
        ...
        if target then
            hm.AutoRotate = false
            HitCircle.Align.Enabled = true
            HitCircle.Align.CFrame = CFrame.lookAt(h.Position, Vector3.new(target.Position.X, h.Position.Y, target.Position.Z))
        else
            HitCircle.Align.Enabled = false
            hm.AutoRotate = true
        end
    end)
...
local function stopHitCircle()
    ...
    if HitCircle.Circle then pcall(function() HitCircle.Circle:Destroy() end); HitCircle.Circle = nil end
    if HitCircle.Align  then pcall(function() HitCircle.Align:Destroy()  end); HitCircle.Align  = nil end
    if HitCircle.Attach then pcall(function() HitCircle.Attach:Destroy() end); HitCircle.Attach = nil end
...
-- SPAM BAT (standalone)
-- ============================================
local SpamBat = { Enabled = false, Conn = nil, LastSwing = 0 }
local SPAMBAT_COOLDOWN = 0.12
...
local function startSpamBat()
    if SpamBat.Enabled then return end
    SpamBat.Enabled = true
    if SpamBat.Conn then SpamBat.Conn:Disconnect() end
    SpamBat.Conn = RunService.Heartbeat:Connect(function()
        if not SpamBat.Enabled then return end
        local c = LP.Character
        if not c then return end
        local bat = findBatTool()
        if not bat then return end
        local hum = c:FindFirstChildOfClass("Humanoid")
        if bat.Parent ~= c and hum then hum:EquipTool(bat) end
        local now = tick()
        if now - SpamBat.LastSwing < SPAMBAT_COOLDOWN then return end
        SpamBat.LastSwing = now
        pcall(function() bat:Activate() end)
    end)
    Notify("SPAM BAT ON")
end
...
local function stopSpamBat()
    if not SpamBat.Enabled then return end
    SpamBat.Enabled = false
    if SpamBat.Conn then SpamBat.Conn:Disconnect(); SpamBat.Conn = nil end
    Notify("SPAM BAT OFF")
end
...
ci = ci + 1
MakeNumberBox(combat, "Grab Range", EnhancedGrab.Radius, ci, function(v) EnhancedGrab.Radius = math.clamp(v,1,100); if grabBarRef.radiusLbl then grabBarRef.radiusLbl.Text = EnhancedGrab.Radius.."st" end; Notify("Grab Range = "..EnhancedGrab.Radius) end, 1, 100, "GrabRadius")
ci = ci + 1
MakeNumberBox(combat, "Grab Speed", EnhancedGrab.Delay, ci, function(v) EnhancedGrab.Delay = math.max(v,0.05); if grabBarRef.rateLbl then grabBarRef.rateLbl.Text = string.format("%.2f", EnhancedGrab.Delay).."s" end; Notify("Grab Speed = "..string.format("%.2f", EnhancedGrab.Delay)) end, 0.05, 3.0, "GrabDelay")
ci = ci + 1
...
ci = ci + 1
MakeToggle(combat, "HIT CIRCLE", ci, function(s) if s then startHitCircle() else stopHitCircle() end end, function() return HitCircle.Enabled end, "HitCircle")
ci = ci + 1
MakeToggle(combat, "SPAM BAT", ci, function(s) if s then startSpamBat() else stopSpamBat() end end, function() return SpamBat.Enabled end, "SpamBat")
ci = ci + 1
ci = ci + 1
...
pi = pi + 1
MakeToggle(protect, "UNWALK (Animations)", pi, function(s) if s then startUnwalk() else stopUnwalk() end end, function() return unwalkActive end, "Unwalk")
pi = pi + 1
...
MakeToggle(visual, "OPTIMIZER", vi, function(s) if s then enableOptimizer() else optimizerEnabled = false; Notify("OPTIMIZER OFF (reload to revert)") end end, function() return optimizerEnabled end, "Optimizer")
vi = vi + 1
...
Notify("H2N v5.2 - Unwalk (Animations Only - Menu Only)")
print("===============================================================")
print("H2N ULTIMATE v5.2")
print("UNWALK: Stops animations only - movement works normally")
print("Access via Menu (Protect tab) or Press U key")
print("===============================================================")