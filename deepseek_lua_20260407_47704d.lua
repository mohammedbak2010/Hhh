...
local EnhancedGrab = { Enabled = false, Radius = 10, Delay = 0.12, CurrentProgress = 0, _timer = 0, LoopConnection = nil }
local grabBarRef = {}
local sbFill = nil
local stealBarFrame = nil

local _grabPromptCache = {}
local _grabCacheTime = 0
local _GRAB_CACHE_TTL = 0.1

local function _grabRefreshCache()
    local now = tick()
    if now - _grabCacheTime < _GRAB_CACHE_TTL then return _grabPromptCache end
    _grabPromptCache = {}
    local plots = workspace:FindFirstChild("Plots")
    if not plots then return _grabPromptCache end
    for _, plot in pairs(plots:GetChildren()) do
        for _, desc in pairs(plot:GetDescendants()) do
            if desc:IsA("ProximityPrompt") and desc.Enabled then
                local isOwn = Char and desc:IsDescendantOf(Char)
                if not isOwn then
                    local pos = nil
                    ...
                    if pos then
                        table.insert(_grabPromptCache, { prompt = desc, position = pos })
                    end
                end
            end
        end
...
local function _execCallbacks(prompt)
...
local function GetClosestPromptEnhanced()
    ...
    local cache = _grabRefreshCache()
    local best = nil
    local bestDist = EnhancedGrab.Radius
    local myPos = HRP.Position
    for _, item in ipairs(cache) do
        if item.prompt and item.prompt.Parent and item.prompt.Enabled then
            local d = (myPos - item.position).Magnitude
            if d < bestDist then
                bestDist = d
                best = item.prompt
            end
...
local function ExecuteStealEnhanced(prompt)
    ...
    _buildCallbacks(prompt)
    if _execCallbacks(prompt) then return true end
    ...
    pcall(function()
        prompt:InputHoldBegin()
        task.delay(0.04, function() pcall(function() prompt:InputHoldEnd() end) end)
    end)
    return true
end
...
local function Load()
    ...
    if d.EnhancedGrab then
        ...
        if d.EnhancedGrab.Delay then EnhancedGrab.Delay = math.clamp(d.EnhancedGrab.Delay, 0.05, 2.0) end
...
ci = ci + 1
MakeNumberBox(combat, "Grab Range", EnhancedGrab.Radius, ci, function(v) EnhancedGrab.Radius = math.clamp(v,5,50); if grabBarRef.radiusLbl then grabBarRef.radiusLbl.Text = EnhancedGrab.Radius.."st" end; Notify("Grab Range = "..EnhancedGrab.Radius) end, 5, 50, "GrabRadius")
ci = ci + 1
MakeNumberBox(combat, "Grab Speed", EnhancedGrab.Delay, ci, function(v) EnhancedGrab.Delay = math.clamp(v,0.05,2.0); if grabBarRef.rateLbl then grabBarRef.rateLbl.Text = string.format("%.2f", EnhancedGrab.Delay).."s" end; Notify("Grab Speed = "..string.format("%.2f", EnhancedGrab.Delay)) end, 0.05, 2.0, "GrabDelay")
ci = ci + 1
